...
struct Polyhedron {
  ...
  void set_lc(const LinearConstraint<Dim>& lc) {
    lc_ = lc;
  }

  /// Set the vertices of the polyhedron
  void set_vertices(const Eigen::Matrix3Xd& vertices) {
    vertices_ = vertices;
  }

  /// Hyperplane array
  vec_E<Hyperplane<Dim>> vs_; // normal must go outside
  LinearConstraint<Dim> lc_;  // Ax < b
  Eigen::Matrix3Xd vertices_; // vertices of the polyhedron
};
